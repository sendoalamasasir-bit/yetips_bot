...
LIGAS = {
    ...
    "🇧🇪 Jupiler Pro League": {"api": "BJL", "csv": "B1"},
    "🇬🇷 Super League": {"api": "G1", "csv": "G1"},
    "🇹🇷 Süper Lig": {"api": "T1", "csv": "T1"},
    "🏴󠁧󠁢󠁳󠁣󠁴󠁿 Premiership": {"api": "SC0", "csv": "SC0"}
}
...
def analizar_partido_manual(local, visita, stats, manual_data):
    ...
    return {
        ...
        "shots_L": shots_L, "shots_V": shots_V, # AÑADIDO
...
def analizar_partido_ia(cerebro, local, visita):
    ...
    return {
        ...
        "shots_L": pred['shoth'], "shots_V": pred['shota'], # AÑADIDO
...
if st.button(f"🚀 ANALIZAR {liga_sel}"):
    ...
    # Obtener Partidos (Quitamos status para ver todo lo Scheduled)
        ...
        if prox:
            ...
            for m in prox:
                ...
                audit.append({
                    ...
                    "Status": "OK" if not res['error'] else "SIN DATOS",
